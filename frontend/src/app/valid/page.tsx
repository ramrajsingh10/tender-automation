...
interface Fact {
  ...
  provenance?: { textAnchors: Array<Record<string, unknown>> };
}
...
export default function ValidationPage() {
  ...
  const refreshData = async (id: string) => {
    const [factRes, annexureRes] = await Promise.all([
      ...
      fetchJson<{ items: Annexure[] }>(`/api/dashboard/tenders/${id}/annexures`),
    ]);
  ...
  const decideFact = async (factId: string, decision: 'approved' | 'rejected') => {
    if (!tenderId) return;
  ...
  const decideAnnexure = async (annexureId: string, decision: 'approved' | 'rejected') => {
    if (!tenderId) return;
  ...
  return (
    <main className='mx-auto flex min-h-screen max-w-5xl flex-col gap-6 px-4 py-16'>
      <header className='space-y-2'>
        ...
        {!tenderId ? (
          <p className='text-sm text-destructive'>Provide a tenderId query parameter to load data.</p>
        ) : null}
      </header>

      {error ? <div className='rounded border border-destructive/60 bg-destructive/10 px-4 py-3 text-sm text-destructive'>{error}</div> : null}
      ...
      <section className='rounded-lg border border-border bg-card p-6 shadow-sm'>
        ...
        {facts.length === 0 ? (
          <p className='text-sm text-muted-foreground'>No facts available yet.</p>
        ) : (
          <ul className='mt-4 space-y-3'>
            {facts.map((fact) => (
              <li key={fact.id} className='rounded border bg-background p-4 shadow-sm'>
                <div className='flex flex-wrap items-center justify-between gap-2'>
                  <div>
                    <p className='font-semibold'>{fact.factType}</p>
                    <p className='text-xs text-muted-foreground'>Confidence: {(fact.confidence ?? 0).toFixed(2)}</p>
                  </div>
                  <div className='text-right'>
                    <span className='block text-xs uppercase tracking-wide text-muted-foreground'>{fact.status ?? 'pending'}</span>
                    {fact.decisionAt ? <span className='block text-[10px] text-muted-foreground'>Decided {new Date(fact.decisionAt).toLocaleString()}</span> : null}
                  </div>
                </div>
                {fact.decisionNotes ? <p className='mt-2 text-xs text-muted-foreground'>Notes: {fact.decisionNotes}</p> : null}
                  ...
                  <div className='mt-3 rounded border bg-muted/40 px-3 py-2 text-xs text-muted-foreground'>
                    ...
                    <ul className='mt-2 list-inside space-y-1'>
                      {fact.provenance.textAnchors.map((anchor, index) => (
                        <li key={index}>
                          Page {anchor.page ?? 'n/a'} — {anchor.snippet ?? 'snippet unavailable'}
                        </li>
                      ))}
                    </ul>
                  </div>
      ...
      <section className='rounded-lg border border-border bg-card p-6 shadow-sm'>
        ...
        {annexures.length === 0 ? (
          <p className='text-sm text-muted-foreground'>No annexure references found.</p>
        ) : (
          <ul className='mt-4 space-y-3'>
            {annexures.map((annexure) => (
              <li key={annexure.id} className='rounded border bg-background p-4 shadow-sm'>
                <div className='flex flex-wrap items-center justify-between gap-2'>
                  <div>
                    <p className='font-semibold'>{annexure.payload.name ?? annexure.annexureType}</p>
                  ...
                  <div className='text-right'>
                    <span className='block text-xs uppercase tracking-wide text-muted-foreground'>{annexure.status ?? 'pending'}</span>
                    {annexure.decisionAt ? (
                      <span className='block text-[10px] text-muted-foreground'>Decided {new Date(annexure.decisionAt).toLocaleString()}</span>
                    ) : null}
                  </div>
                </div>
                {annexure.decisionNotes ? <p className='mt-2 text-xs text-muted-foreground'>Notes: {annexure.decisionNotes}</p> : null}
                <div className='mt-3 flex gap-2'>
                  <button
                    type='button'

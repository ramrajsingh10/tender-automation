...
def get_firestore_client() -> FirestoreClient:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "FIRESTORE_EMULATOR_HOST before starting the deadlines extractor."
        ) from exc
    return _firestore_client
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        description="Extracts tender deadlines from normalized documents.",
        version="0.1.0",
    )
    ...
    async def extract(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        facts = extract_deadlines(document)
        stored_ids = _persist_facts(tender_id, facts)
        logger.info("Extracted %d deadline facts for tender %s.", len(stored_ids), tender_id)

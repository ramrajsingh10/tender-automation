...
try:
    from .extractor import extract_emd
except ImportError:
    from extractor import extract_emd
...
def get_firestore_client() -> FirestoreClient:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "FIRESTORE_EMULATOR_HOST before starting the EMD extractor."
        ) from exc
    return _firestore_client
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="EMD Extractor",
        description="Extracts earnest money deposit information from normalized documents.",
        version="0.1.0",
    )
    ...
    async def extract(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        facts = extract_emd(document)
        stored_ids = _persist_facts(tender_id, facts)
        logger.info("Extracted %d EMD facts for tender %s.", len(stored_ids), tender_id)

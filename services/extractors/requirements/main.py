...
def get_firestore_client() -> FirestoreClient:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "FIRESTORE_EMULATOR_HOST before starting the requirements extractor."
        ) from exc
    return _firestore_client
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        description="Extracts technical and financial requirements from normalized documents.",
        version="0.1.0",
    )
    ...
    async def extract(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        facts = extract_requirements(document)
        stored_ids = _persist_facts(tender_id, facts)
        logger.info("Extracted %d requirement facts for tender %s.", len(stored_ids), tender_id)
...
def _persist_facts(tender_id: str, facts: list[dict[str, Any]]) -> list[str]:
    ...
    for fact in facts:
        ...
        doc = {
            ...
            "version": 1,
            "createdAt": now,
            "createdBy": "extractor.requirements",
        }
        doc_ref.set(doc)
